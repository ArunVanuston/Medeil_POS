...
export class viewdrugComponent implements OnInit, AfterViewInit {
 ...
 // public indx;
  ...
  ngOnInit() {
    ...
    this.searchindex=1;
    //this.indx=0;  
    this.viewdrug.getAlldrugInfo(AppComponent.companyID).subscribe(data =>{ 
  ...
  productdetails(drugdetails){
    ...
    this.viewdrug.getCustDrugValues(drugdetails[0]).subscribe(data => {
    ...
    //Get Combination Generic
    setTimeout(() => {
      this.viewdrug.getCombineGenericValues(drugdetails[0]).subscribe(data => {this.combinegeneric=data
      },error => {
        console.log('Error occured in Drug getdrugEditvalues');
      });
    },1000);
