...
export class PrescriptionApprovalService {
  ...
  baseResUrl: string = environment.backend.baseResUrl;
    ...
    getsolist(cid: number, bid: number, loc: number, lrid: number) {
      this.headertoken();
        return this.http.get(this.baseResUrl+'/getprescsalesorder' + '/' + cid + '/' + bid + '/' + loc + '/' + lrid,
        {headers: this.head} ).map(response => response.json());
      }

      getsearchproduct(cid: number, bid: number, loc: number, lrid: number,val:any) {
        this.headertoken();
        return this.http.get(this.baseResUrl+'/getprescprodetail' + '/' + cid + '/' + bid + '/' + loc + '/' + lrid+'/'+val,
        {headers: this.head} ).map(response => response.json());
      }
      ...
      getprescriptionlist(cid: number, bid: number, loc: number, lrid: number) {
        this.headertoken();
        return this.http.get(this.baseResUrl+'/prescdigino' + '/' + cid + '/' + bid + '/' + loc + '/' + lrid, {headers: this.head} ).map(response => response.json());
      }   

      getprescriptionapprlist(cid: number, bid: number, loc: number, lrid: number) {
        this.headertoken();
        return this.http.get(this.baseResUrl+'/prescdiginoapprv' + '/' + cid + '/' + bid + '/' + loc + '/' + lrid, {headers: this.head} ).map(response => response.json());
      }  

      getprescripfield(cid: number, bid: number, loc: number, lrid: number, pres: number) {
        this.headertoken();
        return this.http.get(this.baseResUrl+'/prescdigichecking' + '/' + cid + '/' + bid + '/' + loc + '/' + lrid + '/' +pres,
        {headers: this.head} ).map(response => response.json());
      } 

      getprescrippro(cid: number, bid: number, loc: number, lrid: number, pres: number) {
        this.headertoken();
        return this.http.get(this.baseResUrl+'/presccheckingpro' + '/' + cid + '/' + bid + '/' + loc + '/' + lrid + '/' +pres,
        {headers: this.head} ).map(response => response.json());
      } 
      ...
      checkprescptionlist(data: String) {
        ...
        return this.http.post(this.baseResUrl+'/checkprescdigi', data, { headers: head }).map(response => response.json())
      }

      checkprescptionprod(data: String) {
       this.headertoken();
        return this.http.post(this.baseResUrl+'/checkprescdigitable', data, { headers: this.head }).map(response => response.json())
      }

      viewprescription(cid: number, bid: number, loc: number, lrid: number) {
        this.headertoken();
        return this.http.get(this.baseResUrl+'/viewprescriptio' + '/' + cid + '/' + bid + '/' + loc + '/' + lrid, {headers: this.head} ).map(response => response.json());
      }

      getprescriptionhistory(cid: number, bid: number, loc: number, lrid: number) {
        this.headertoken();
        return this.http.get(this.baseResUrl+'/viewprescriptio' + '/' + cid + '/' + bid + '/' + loc + '/' + lrid, {headers: this.head} ).map(response => response.json());
      }
