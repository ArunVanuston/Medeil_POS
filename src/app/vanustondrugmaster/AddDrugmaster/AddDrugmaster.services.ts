...
export class adddrugService {
  ...
  private drugUrl = this.baseResUrl+'/vanuston-drugcreateRecord';
  ...
  gettaxCountrystate(countryid:any) {
    this.headertoken();
    return this.http.get(this.baseResUrl+'/gettaxcountrywise/'+countryid,{ headers: this.head }).map(res => res.json());
  }
  ...
  saveGenericname(serobj: string) { 
    ...
    return this.http.post(this.saveGenericnameURL, serobj, { headers: this.head })  .map((res: Response) => res.json());
  ...
  saveVertical(data: String) {
    this.headertoken();
    return this.http.post(this.baseResUrl+'/vanuston-save-vertical-drop-down', data, { headers: this.head }).map(response => response.json())
  }
