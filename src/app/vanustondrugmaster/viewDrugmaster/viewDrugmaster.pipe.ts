...
export class FilterPipe implements PipeTransform {
  transform(items: any[], filterQuery: string, filterQuery1: string, filterQuery2: string): any {
      //return items.filter(item => item[1].toLowerCase().indexOf(filterQuery) !== -1);   //singlesearch
      return items.filter(item =>{
        if (filterQuery && item[1].toLowerCase().indexOf(filterQuery.toLowerCase()) === -1){
            return false;
        }
        if (filterQuery1 && item[2].toLowerCase().indexOf(filterQuery1.toLowerCase()) === -1){
            return false;
        }
        if (filterQuery2 && item[7].toLowerCase().indexOf(filterQuery2.toLowerCase()) === -1){
          return false;
        }
        return true;
      })
