  ...
  getpurchaseinvoice(cid:any, bid:any, locname:any, locrefid:any){
   ...
   getBranch(cid: any) {
    this.headertoken();
     ...
     getDistributorInfo(cid:any,bid:any,lname:any,lrefid:any)    {
      this.headertoken();
       return this.http.get(this.getdistinfo+'/'+cid+'/'+bid+'/'+lname+'/'+lrefid,{headers: this.head}).map(response => response.json());
     }
     getCountry() {
      this.headertoken();
